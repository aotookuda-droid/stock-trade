...
ENTRIES_RAW = [
    ...
    # ── 実力上位 ──
    ("ドウデュース",         6, "牡", 470, "武豊",      "友道康夫", "栗東",  97, 91, 97, 86,  2,  8),
    ("ジャスティンパレス",   5, "牡", 478, "岩田望来",  "杉山晴紀", "栗東",  91, 95, 88, 84,  4,  7),
    ("ベラジオオペラ",       5, "牡", 472, "横山和生",  "上村洋行", "栗東",  88, 88, 89, 85,  1,  8),
    ("スターズオンアース",   6, "牝", 446, "ルメール",  "高柳大輔", "美浦",  91, 87, 93, 90,  0,  7),
    ("ブローザホーン",       6, "牡", 462, "菅原明良",  "吉岡辰弥", "栗東",  86, 96, 80, 83,  9,  9),
    ("ローシャムパーク",     5, "牡", 490, "戸崎圭太",  "高木登",   "美浦",  85, 90, 84, 80,  5,  6),
    ("プログノーシス",       6, "牡", 496, "中山雄太",  "斉藤崇史", "栗東",  87, 92, 87, 81,  2,  5),
    # ── 3歳・若手 ──
    ("クロワデュノール",     3, "牡", 466, "北村友一",  "木村哲也", "美浦",  89, 85, 93, 82,  1, 10),  # 日本ダービー馬
    ("メイショウタバル",     4, "牡", 484, "浜中俊",    "石坂公一", "栗東",  84, 90, 85, 79,  3,  9),  # 大阪杯好走
    # ── 中堅 ──
    ("シュトルーヴェ",       5, "牡", 480, "池添謙一",  "池江泰寿", "栗東",  83, 89, 82, 78,  2,  5),
    ("ヒシイグアス",         8, "牡", 488, "松山弘平",  "堀宣行",   "美浦",  80, 91, 79, 84,  3,  4),
    ("ディープボンド",       7, "牡", 486, "幸英明",    "大久保龍", "栗東",  78, 97, 76, 85,  6,  3),
    ("ハーパー",             5, "牝", 436, "岩田康誠",  "松永幹夫", "栗東",  81, 84, 83, 83,  0,  6),
    ("ルージュエヴァイユ",   5, "牝", 442, "川田将雅",  "黒岩陽一", "栗東",  82, 83, 86, 82,  0,  7),
    ("サトノグランツ",       5, "牡", 464, "坂井瑠星",  "池添兼雄", "栗東",  81, 88, 81, 77,  1,  5),
    ("ヴェルトハイム",       5, "牡", 474, "藤岡佑介",  "中竹和也", "栗東",  80, 86, 83, 80,  1,  4),
    ("エコロヴァルツ",       4, "牡", 454, "横山武史",  "安田翔伍", "栗東",  79, 84, 82, 76,  0,  6),
    ("アレグロモデラート",   4, "牝", 438, "三浦皇成",  "高橋康之", "美浦",  76, 82, 80, 74,  1,  5),
]
...
def run_simulation(seed: Optional[int] = None) -> dict:
    ...
    print("=" * 62)
    print("  2025年 宝塚記念 (G1)  シミュレーション")
    print("  阪神競馬場 芝2200m (外回り) 2025年6月29日(日)")
    print("=" * 62)
